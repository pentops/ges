...
const (
	FooPublishTopic_FooEvent_FullMethodName = "/gestest.v1.topic.FooPublishTopic/FooEvent"
)
...
var FooPublishTopic_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "gestest.v1.topic.FooPublishTopic",
	...
	Metadata: "gestest/v1/topic/foo.p.j5s.proto",
}

const (
	FooSummaryTopic_FooSummary_FullMethodName = "/gestest.v1.topic.FooSummaryTopic/FooSummary"
)
...
var FooSummaryTopic_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "gestest.v1.topic.FooSummaryTopic",
...
const (
	GenericTopic_Foo_FullMethodName = "/gestest.v1.topic.GenericTopic/Foo"
)

// GenericTopicClient is the client API for GenericTopic service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type GenericTopicClient interface {
	Foo(ctx context.Context, in *FooMessage, opts ...grpc.CallOption) (*emptypb.Empty, error)
}

type genericTopicClient struct {
	cc grpc.ClientConnInterface
}

func NewGenericTopicClient(cc grpc.ClientConnInterface) GenericTopicClient {
	return &genericTopicClient{cc}
}

func (c *genericTopicClient) Foo(ctx context.Context, in *FooMessage, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, GenericTopic_Foo_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// GenericTopicServer is the server API for GenericTopic service.
// All implementations must embed UnimplementedGenericTopicServer
// for forward compatibility
type GenericTopicServer interface {
	Foo(context.Context, *FooMessage) (*emptypb.Empty, error)
	mustEmbedUnimplementedGenericTopicServer()
}

// UnimplementedGenericTopicServer must be embedded to have forward compatible implementations.
type UnimplementedGenericTopicServer struct {
}

func (UnimplementedGenericTopicServer) Foo(context.Context, *FooMessage) (*emptypb.Empty, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Foo not implemented")
}
func (UnimplementedGenericTopicServer) mustEmbedUnimplementedGenericTopicServer() {}

// UnsafeGenericTopicServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to GenericTopicServer will
// result in compilation errors.
type UnsafeGenericTopicServer interface {
	mustEmbedUnimplementedGenericTopicServer()
}

func RegisterGenericTopicServer(s grpc.ServiceRegistrar, srv GenericTopicServer) {
	s.RegisterService(&GenericTopic_ServiceDesc, srv)
}

func _GenericTopic_Foo_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(FooMessage)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(GenericTopicServer).Foo(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: GenericTopic_Foo_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(GenericTopicServer).Foo(ctx, req.(*FooMessage))
	}
	return interceptor(ctx, in, info, handler)
}

// GenericTopic_ServiceDesc is the grpc.ServiceDesc for GenericTopic service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var GenericTopic_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "gestest.v1.topic.GenericTopic",
	HandlerType: (*GenericTopicServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Foo",
			Handler:    _GenericTopic_Foo_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "gestest/v1/topic/foo.p.j5s.proto",
